...
#run deployment in each subscription
    ...
    foreach ($subscription in $subscriptions) {
        ...
        if ($testDeploy) {
            ...
            New-AzSubscriptionDeploymentStack -Name $deploymentName -Location "WestUS3" -TemplateFile $bicepFile -DenySettingsMode "None" -templateParameterObject @{clientCode = $clientCode} -Verbose -Force -Confirm:$false
        }
