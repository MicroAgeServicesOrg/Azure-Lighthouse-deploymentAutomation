...
$bicepFile = ".\azMSP_baselinePolicy\configure_log_analytics_custom\policyDeploy.bicep"
...
if ((!$subscriptionId) -and ($subscriptions)) {
    ...
    foreach ($subscription in $subscriptions) {
        ...
        New-AzSubscriptionDeployment -Name "devops_deployLogPolicy" -Location "WestUS2" -TemplateFile $bicepFile
    }
}
